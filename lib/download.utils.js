...
function renderDownloaderProgress(percentage, currentTitleLength, maxTitleLength, marginCorrection = 0) {
  ...
  if (percentage === 1) {
    return ' '.repeat(maxTitleLength - currentTitleLength - marginCorrection) + `[` + dots + spaces + `] ${copy}/100%`
  }

  return ' '.repeat(maxTitleLength - currentTitleLength) + `[` + dots + spaces + `] ${copy}/100%`
}